        ...
        get_data=new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    URL url=new URL("http://"+Api.url+"tagam24/get_banner.php");
                    Log.d("url",""+url);
            ...
            public void run() {
                try{//bashda data basadan hemme informasiyasyny alyas
                    ...
                    for(int i=0; i<jsonArray.length();i++) {
                        ...
                        Constants.banner.add(new Kafe_bar_mod
                                (data.optString("id"),data.optString("name"),data.optString("image"),data.optString("image1"),
                                        data.optString("image2"),data.optString("image3"),data.optString("content"),
                                        data.optString("watch"),data.optString("n_people"),data.optString("s_people"),
                                        data.optString("rating"),data.optString("address"),data.optString("work_time"),
                                        data.optString("work_data"),data.optString("karta_image"),data.optString("number")));
