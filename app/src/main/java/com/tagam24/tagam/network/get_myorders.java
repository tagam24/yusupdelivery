        ...
        get_data=new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    URL url=new URL("http://"+Api.url+"tagam24/get_orderID.php");
                    Log.d("url",""+url);
                    ...
                    String post_data = URLEncoder.encode("token", "UTF-8") + "=" + URLEncoder.encode(FirebaseInstanceId.getInstance().getToken(), "UTF-8")+"&"+
            ...
            public void run() {
                try{//bashda data basadan hemme informasiyasyny alyas
                    ...
                    for(int i=0; i<jsonArray.length();i++) {
                        ...
                        Constants.orderid.add(
                                new Model_myorder1(data.optString("id"),data.optString("cafeName"),data.optString("tt_price"),
                                        data.optString("date"),data.optString("accepted"),data.optString("place")));
                    }
                    Log.d("Alimerdan","size:"+Constants.food.size()+"geldi:"+s);

                    myorder1.s1.sendEmptyMessage(1);
               Log.d("conn",""+con);
                    Thread.currentThread().interrupt();
