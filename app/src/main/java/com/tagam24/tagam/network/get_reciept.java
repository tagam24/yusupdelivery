...
import com.tagam24.tagam.gumenje.Reciept;
        ...
        get_data=new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    URL url=new URL("http://"+Api.url+"tagam24/get_reciepts.php");
                    Log.d("url",""+url);
                ...
                try{//bashda data basadan hemme informasiyasyny alyas
                    ...
                    for(int i=0; i<jsonArray.length();i++) {
                        JSONObject data = jsonArray.getJSONObject(i);
                                ...
                                Constants.recepts.add(new recept(data.optString("id"),
                                    data.optString("name"),data.optString("image"),
                                    data.optString("image1"),data.optString("image2"),data.optString("liked"),
                                    data.optString("n_people"),
                                    data.optString("rating"),data.optString("watch"),data.optString("content"),
                                    data.optString("text1"),data.optString("text2"),data.optString("date")));
                    }
                    Reciept.s1.sendEmptyMessage(1);
                    Thread.currentThread().interrupt();
