...
import com.tagam24.tagam.aksiya.fragment_arzanlad;
        ...
        get_data=new Thread(new Runnable() {
            ...
            public void run() {
                try{
                    ...
                    URL url=new URL("http://"+Api.url+"tagam24/get_foodaksiya.php");
                    Log.d("url",""+url);
            ...
            public void run() {
                try{//bashda data basadan hemme informasiyasyny alyas
                    ...
                    for(int i=0; i<jsonArray.length();i++) {
                      ...
                      m=new Model_Food(data.optString("id"),data.optString("image"),data.optString("image1"),
                      ...
                      m.setAksiya(data.optString("aksiya"));
                        Constants.food.add(m);
                    }
                    Log.d("Alimerdan","size:"+Constants.food.size()+"geldi:"+s);
                    arzanlad_menu.s1.sendEmptyMessage(1);
                    Log.d("conn",""+con);
                    Thread.currentThread().interrupt();
